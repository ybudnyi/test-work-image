...
options {
      ...
      stages {
        ...
        stage('PUSH IMAGES') {
            steps {
                script {
                    sh "/usr/local/bin/docker login ghcr.io -u $ghcr_user -p ${VCS_TOKEN}"
                    // sh (script: "/bin/bash -c 'echo ${VCS_TOKEN} | /usr/local/bin/docker login ghcr.io -u ybudnyi --password-stdin'")
